<template>
  <mr-container
    ...
    @redo="$root.$emit('redo')"
  >
    <stage-el v-for="element in page.children" :key="element.id" :elem="element"></stage-el>
  </mr-container>
</template>
...
export default {
  ...
  computed: {
    pageStyles() {
      return {
        ...this.page.styles,
        height:
          typeof this.page.height === "string"
            ? this.page.height
            : this.page.height + "px",
        width:
          typeof this.page.width === "string"
            ? this.page.width
            : this.page.width + "px",
  ...
  methods: {
    ...
    dropHandler(e) {
      ...
      const fixedElement = fixElementToParentBounds(
    ...
    selectStopHandler(selectionBox) {
      if (
        (selectionBox.top === selectionBox.bottom &&
          selectionBox.left === selectionBox.right) ||
      ...
      this.page.children.forEach(childEl => {
        const child = childEl.global
          ? { ...childEl, ...this.getComponentRef(childEl), id: childEl.id }
          : childEl;

        let childTop = getComputedProp("top", child);
        let childLeft = getComputedProp("left", child);
        let childBottom =
          getComputedProp("height", child, this.page) + childTop;
        let childRight = getComputedProp("width", child, this.page) + childLeft;

        if (
          (childTop <= selectionBox.bottom &&
            childLeft <= selectionBox.right &&
            childBottom >= selectionBox.top &&
            childRight >= selectionBox.left) ||
          (childTop <= selectionBox.bottom &&
            childRight >= selectionBox.left &&
            childBottom >= selectionBox.top &&
            childLeft <= selectionBox.right)
    ...
    getComponentRef(component) {
      return this.projectComponents[
        this.projectComponents.findIndex(comp => comp.name === component.name)
      ];
    },

    resizeStopHandler(resStopData) {
      resStopData.map(resElData =>
        this.resizeElement({ ...resElData, pageId: this.page.id })
      );
    ...
    arrowsHandler({ direction, shiftKey }) {
      if (this.selectedElements.length > 0) {
        ...
        this.selectedElements.map(el => {
          ...
          console.log(compTop, compBottom, compLeft, compRight);
          let top =
          ...
          let bottom =
            addedBottom &&
            compBottom + addedBottom >= 0 &&
            compTop + addedTop >= 0
              ? compBottom + addedBottom
              : null;
          let left =
            addedLeft &&
            compLeft + addedLeft >= 0 &&
            compRight + addedRight >= 0
              ? compLeft + addedLeft
              : null;
          let right =
            addedRight &&
    ...
    getContaineggOnPoint(x, y) {
      ...
      const parentsIds = movingEggs.map(egg =>
        egg.id.substring(0, egg.id.lastIndexOf("."))
      );
      const commonParentId = parentsIds.every((val, i, arr) => val === arr[0])
        ? parentsIds[0]
        : null;
      ...
      for (let el of elementsOnPoint) {
        ...
        if (
          el.dataset.mrContainer ||
          (el.dataset.containegg &&
            !el.dataset.componegg &&
            movingEggs.every(egg => !(el.id || el.dataset.id).includes(egg.id)))
        )
          return el;
      }
      return null;
