<template>
  <div
    ...
    @keydown.up="e => $emit('arrows', {direction: 'up', shiftKey: e.shiftKey})"
    @keydown.down="e => $emit('arrows', {direction: 'down', shiftKey: e.shiftKey})"
    @keydown.left="e => $emit('arrows', {direction: 'left', shiftKey: e.shiftKey})"
    @keydown.right="e => $emit('arrows', {direction: 'right', shiftKey: e.shiftKey})"
...
export default {
  ...
  computed: {
    mrElements() {
      return this.activeElements.map(el => {
        console.log("el", el);
        let ele = document.querySelector(`[data-id="${el.id}"]`);

        if (!ele) {
          ele = document.getElementById(el.id);
        }
        if (ele.parentElement.dataset.mrEl) {
  ...
  methods: {
    ...
    mouseMoveHandler(e) {
      ...
      if (this.resizing) {
        this.mrElements.map(mrEl => {
          if (!mrEl.children[0].dataset.global)
            this.resizeElementBy(mrEl, offX, offY);
        });
        // this.$emit('resizing')
      } else if (this.moving) {
    ...
    renderSelectionArea(initPoint, endPoint) {
      ...
      this.$refs.selectionArea.style.width =
        Math.round((maxX - minX) / this.zoom) + "px";
      this.$refs.selectionArea.style.height =
        Math.round((maxY - minY) / this.zoom) + "px";
    },

    resizeElementBy(el, offX, offY) {
      const parentCompStyle = window.getComputedStyle(this.getParentMr(el));

      console.log("parentCompStyle", parentCompStyle);
      ...
      if (this.handle.indexOf("b") !== -1) {
        ...
        else if (newTop + newHeight + offY > parentH)
          diffY = parentH - newTop - newHeight;
        newHeight += Math.round(diffY / this.zoom);
        newBottom -= Math.round(diffY / this.zoom);
      }
      if (this.handle.indexOf("r") !== -1) {
        if (newWidth + offX < elMinW) diffX = elMinW - newWidth;
        else if (newLeft + newWidth + offX > parentW)
          diffX = parentW - newLeft - newWidth;
        newWidth += Math.round(diffX / this.zoom);
    ...
    moveElementBy(el, offX, offY) {
      ...
      el.style.top =
        el.style.top !== "auto"
          ? this.fixPosition(
              el,
              parseInt(elCompStyle.top) + Math.round(offY / this.zoom),
              "top"
            ) + "px"
          : "auto";
      el.style.left =
        el.style.left !== "auto"
          ? this.fixPosition(
              el,
              parseInt(elCompStyle.left) + Math.round(offX / this.zoom),
              "left"
            ) + "px"
          : "auto";
      el.style.bottom =
        el.style.bottom !== "auto"
          ? this.fixPosition(
              el,
              parseInt(elCompStyle.bottom) - Math.round(offY / this.zoom),
              "bottom"
            ) + "px"
          : "auto";
      el.style.right =
        el.style.right !== "auto"
          ? this.fixPosition(
              el,
              parseInt(elCompStyle.right) - Math.round(offX / this.zoom),
              "right"
            ) + "px"
          : "auto";
    },
    ...
    getParentMr(element) {
      ...
      while (parentMr === null) {
        ...
        else if (currentMr.parentElement.dataset.mrEl)
      ...
      return {
        moveElData: this.mrElements.map(el => {
          return {
            elId:
              el.children[0].id ||
              el.children[0].dataset.id ||
              el.children[0].children[0].id ||
              el.children[0].children[0].dataset.id,
            top:
    ...
    selectStopData() {
      return {
        ...
        bottom:
          parseInt(this.$refs.selectionArea.style.height) +
          parseInt(this.$refs.selectionArea.style.top),
        left: parseInt(this.$refs.selectionArea.style.left),
        right:
          parseInt(this.$refs.selectionArea.style.width) +
          parseInt(this.$refs.selectionArea.style.left)
      };
    }
  }
};
