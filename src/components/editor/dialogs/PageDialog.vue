...
export default {
  ...
  computed: {
    dialogTitle() {
      return this.pageDialog.isNew
        ? "add new page"
        : "edit:" + this.activePage.name;
    },

    ...mapState({
      activePage: state => state.app.selectedPage,
      pageDialog: state =>
        state ? state.app.pageDialog : { isNew: true, isOpen: false }
    }),
  ...
  methods: {
    checkName() {
      if (!this.pageDialog.isNew && this.name === this.activePage.name) {
        ...
        this.nameError = "page name needed";
      } else if (!this.name.match(/[a-z]+/g)) {
        this.nameError = "page name need index with letter";
      } else if (this.nameInUse(this.name)) {
        this.nameError = "page name exists";
      } else {
        this.nameError = "";
      }
    ...
    checkPath() {
      if (!this.pageDialog.isNew && this.path === this.activePage.path) {
        ...
        this.pathError = "page path needed";
      } else if (this.path[0] !== "/") {
        this.pathError = "page path need index with /";
      } else if (!this.path.match(/^\/([A-Za-z0-9/_-])+$/g)) {
        this.pathError = "page path invalid";
      } else if (this.pathInUse(this.path)) {
        this.pathError = "page path exists";
      } else {
        this.pathError = "";
      }
      this.valid = this.name && !this.nameError && this.path && !this.pathError;
