...
export function getComputedProp (prop, element, parent) {
  if (prop === 'left' || prop === 'right' || prop === 'top' || prop === 'bottom') {
    let parentNode = document.getElementById(element.id).parentNode
    if (element.external === 'react') {
      parentNode = parentNode.parentNode.parentNode
    }
    return parseInt(window.getComputedStyle(parentNode)[prop])
  } else {
    return (
      (!parent)
        ? parseInt(document.getElementById(element.id) && window.getComputedStyle(document.getElementById(element.id))[prop])
