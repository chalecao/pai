...
export function getChildNode (currentNode, targetId) {
  if (currentNode.id === targetId || (currentNode.dataset && currentNode.dataset.id === targetId)) return currentNode

  for (let child of currentNode.children) {
    if (targetId.indexOf(child.id || (child.dataset && child.dataset.id)) !== -1) {
...
export function calcRelativePoint (currentNode, targetId, currentX, currentY) {
  if (currentNode.id === targetId || (currentNode.dataset && currentNode.dataset.id === targetId)) return { left: currentX, top: currentY }
